...
- ### Alternative Outlier Rejection
  Outliers are rejected per AOR by rejecting the maximum and minimum flux values if the spread of distribution is greater than 2% and the AOR has at least 10 files to work with. This is the standard procedure for outlier rejection that the pipeline is using currently.
  
  However, if you wish to do sigma clipping, you can do so using the `functions.py` script:
  ```python
...
- ### Run Summary
  Run summary for each individual run can be easily obtained by running the *run_summar.sh* file in the "Reductions_Data_&_Logs" folder in this is repository. The *comments* parameter gives a summary of each individual run.
  Here, I still provide the summary of the first 12 runs since they can be easily grouped. For the first 12 runs, I have run the pipeline for 4 groups of data: cryogenic mission data for HD 165459, warm mission data for HD 165459, cryogenic mission data for BD +60 1753, warm mission data for BD +60 1753. So the runs can be grouped into sets of 4. I will also provide the summary of the latest runs for each data set.
  - Run 1-4 were test runs. No outliers were rejected and warm mission data did not have mission-specific systematics applied.
  - Run 5-8 are proper runs for channel 1 data. Which means mission-specific systematics were applied and outliers were rejected in these runs.These are also the runs that I used to create light curves for these stars. George Rieke also used these runs to do his analysis on these stars.
  ...
  - hd_final_cryo, hd_final_warm, bd_final_cryo and bd_final_warm are the same group of 4 as run 1-12. They were run the same way as run 5-8 but with the latest version of the pipeline. The results as consistent with the previous runs.
  - ngc2420_final was run for some selected solar analogs in NGC 2420. It was run with a radius combination of 3, 3, 7. The result has a high spread in distribution.
  - ngc2506_final was run for some selected solar analogs in NGC 2506. There were not a lot of files to work with and so the standard deviation and spread is not reliable. There is one negative flux and it is because the aperture found no star at the given coordinate. Which means the aperture sum turned out to be the sum of some background values.
  - michaela_final and upperSco_final were run with the latest version of the pipeline in the same way. A radius combination of 5, 5, 10 was used. sub2d files were used for getting wcs information while a median of the last 50 frames of bcd file was used for photometry. These are subarray data and as expected, the stars are very bright. Upper Sco stars are from PID 20069 and Michala's list of stars are from PID 70076.
  - taurus_final was run with the latest version of the pipeline using 3, 3, 7 radius combination. this is different from any of the previous runs for Taurus. There are quite a few negative fluxes and it is the same reason as discussed for ngc2505_final. The spread in distribution is quite high and the number of workable files for each star is very low.
...
- ### IMG Table
  ...
  - **Bad Center Guess**: Flags 'Y' if gaussian fitted center deviates by 2 px from provided target coordinates. In the case that aperture is out of bound and the center guess is also bad, only ap_out_of_bound willbe flagged 'Y' and bad_center_guess will be flagged 'N' even though it should be flagged 'Y'.
  - **Not In FOV**: Flags 'Y' if the PSF center lies beyond the detector array limits. This is flagged based on the given approximate centroid. So, if the given centroid and fitted centroid are so far off that the given centroid is in FOV while the fitted center is not in FOV, it will still flag Not_in_FOV as 'N' since it depends on the given centroid. 
