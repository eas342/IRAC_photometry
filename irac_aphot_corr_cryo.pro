;+
; NAME: IRAC_APHOT_CORR_CRYO
;
;
;
; PURPOSE:     Correct IRAC observed aperture photometry-derived fluxes (from the cryogenic mission) for the 
;              pixel phase and array location dependent response functions.  ***NOTE:  the pixel phase response
;              is unity (zero variation across a pixel) for channels 3 and 4.
;              
...
;
;
; CALLING SEQUENCE:   corrected_flux = IRAC_APHOT_CORR_CRYO ( OBSERVED_FLUX, X, Y, CHANNEL $
...
;
;
; EXAMPLE:   IDL> corrected_flux = IRAC_APHOT_CORR_CRYO( OBSERVED_FLUX, X, Y, CHANNEL )
;
;
;
...
FUNCTION irac_aphot_corr_cryo,flux,x,y,channel,params=params,pp_only=pp_only,al_only=al_only,reach=reach,hora=hora,$
                              subarray=subarray
...
FOR i = 0,nch-1 DO BEGIN
   ...
   IF N NE 0 THEN corrected_flux[index] = flux(index)/APHOT_PIXPHASE_POSDEP_FUNC(X[index]+position_offset[0,chan_uniq[i]-1],$
